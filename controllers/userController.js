...
admin.initializeApp({
  ...
  databaseURL: "https://quize-3ea57.firebaseio.com",
});

const postOtpLogin = async (req, res) => {
  try {
    const findUserData = await findUserByMobileNumber(req.body.mobileNumber)

    if (findUserData.length === 0) {
      const dbData = {
        mobileNumber: req.body.mobileNumber,
      }
      const addUser = await otpLogin(dbData)
      const tknData = {
        id: addUser.insertId,
        mobileNumber: req.body.mobileNumber,
        type: 'user'
      ...
      return res.status(200).json(successmessage({ token: authToken, mobileNumber: req.body.mobileNumber }));
    } else {
      const tknData = {
        id: findUserData[0].user_id,
        mobileNumber: findUserData[0].mobileNumber,
        type: findUserData[0].type
      }
      const authToken = await jwt.sign(tknData, JWT_SECRET, {
        expiresIn: "24h",
      });
      return res.status(200).json(successmessage({ token: authToken, mobileNumber: findUserData[0].mobileNumber }));
    }
  } catch (error) {
