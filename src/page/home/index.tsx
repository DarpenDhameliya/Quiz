...
import cardbg from "../../asset/logo/science.png";
...
const Home = () => {
  ...
  const CardView = useCallback(() => {
    ...
    if (playedQuizid && !userfind) {
      ...
      const playQuiz =
        quizList &&
        quizList.response.filter((data: any) => {
          if ((playedDate && parseInt(playedDate)) === new Date().getDate()) {
            return (
              data.live !== 0 && !parsePlayQuizid.includes(String(data.id))
            );
          } else {
      ...
      return (
        <>
          {Object.keys(quizList).length > 0 &&
            quizList.status === "ok" &&
            quizList.response.map((data: any) => {
        ...
        <div className={classes.middleportion}>
          ...
          Object.keys(quizList).length === 0 ? (
            ...
            <Slider {...settings} className="custom-slider">
              {Object.keys(categoryList).length > 0 &&
                categoryList.status === "ok" &&
                categoryList.response.map((data: category) => {
