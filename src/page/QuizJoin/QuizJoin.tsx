...
const QuizJoin: React.FC = () => {
    ...
    const { data: questionList, isLoading, isFetching, refetch } = useQuery(
        ['get-question-list'],
                        ...
                        {questinsList?.answer.map((option, index) => (
                            <div
                                ...
                                className={`${classes.opetions} ${correctAns !== null && correctAns === index && classes.correctanscss}
                                 ${wrongans !== null && wrongans === index && classes.wronganscss} ${(correctAns !== null && wrongans !== null) && (correctAns !== index && wrongans !== index) && 'setopacitystyle'}`}
            ...
            if (progress === 100 || progress >= 100 || AppliedAns + 1 === 16) {
                ...
                if (!userfind) {
                    if (sessionStorage.getItem('coin') === null) {
                        ...
                        if (userWallet) {
                            sessionStorage.setItem('coin', (parseInt(userWallet) + coin).toString());
                        }
                    }
    ...
    const callFifty = () => {
        const correctAnswerIndex = questinsList?.answer.indexOf(questinsList?.correct);

        let modifiedOptions: any = [];
        if (questinsList) {
            const filterAnotherOpetion = questinsList?.answer.filter((data: any, index) => index !== correctAnswerIndex)
            let filterOpetionId = Math.floor(Math.random() * filterAnotherOpetion.length);
            const secondOpetion = questinsList?.answer.indexOf(filterAnotherOpetion[filterOpetionId])
        ...
        <Paper className={classes.setProductpape} elevation={5}>
                    ...
                    : <>
                        ...
                        >
                            {`Time Remaining: ${Math.max(
                                0,
                                Math.floor(parseInt(quizTimer) - (progress / 100) * parseInt(quizTimer))
                            )} seconds`}
                        </Typography>
                        <Typography
                            variant="h5"
                        ...
                        {QuestionView()}
                        <span className="flex justify-center mt-3" style={{ color: "var(--whitebglight-color)" }}>Your Score : <span className="ml-2" style={{ color: "var(--text-color)" }}>{totalScore}</span></span>
                        <div className='d-flex justify-center ads-box' style={{ marginTop: 20, marginBottom: '50px' }}>
                            <img src={adsImg} alt='ad' style={{
                                width: '100%',
                                maxHeight: '320px',
                            }} />
                        </div>
            ...
            <div className={`bottom-fixed-div ${openLifeline ? 'visible' : ''}`}>
                ...
                <div className="flex justify-center w-full h-[40px] ">
                    <div onClick={handleOpenLifeline} className="lifelinebtn l-0 relative border-border border-[1px] rounded-full  px-3 py-2 flex gap-2 justify-center items-center absolute top-0 bg-bg_nav text-sm cursor-pointer transition-all duration-500 min-w-[30%]" >
                        <span> Tap to use Lifelines </span>
                    </div>
                </div>
                <div className="flex mt-4 text-[12px] items-start justify-evenly transition-all duration-1000 h-[0] overflow-hidden" style={{ height: 'auto' }}>
                    <div className={`w-full  flex flex-col absolute items-center justify-evenly text-base bg-bg_nav ${questinsList?.audience ? '' : 'hidden'}`}>
                        <div className="flex w-[50%] justify-evenly">
                            <div className="flex flex-col justify-center items-center gap-1 text-slate-300	">A - {questinsList && questinsList.audience && questinsList.audience[0]} %</div>
                            <div className="flex flex-col justify-center items-center gap-1 text-slate-300	">B - {questinsList && questinsList.audience && questinsList.audience[1]} %</div>
                        </div>
                        <div className="flex w-[50%] justify-evenly">
                            <div className="flex flex-col justify-center items-center gap-1 text-slate-300">C - {questinsList && questinsList.audience && questinsList.audience[2]} %</div>
                            <div className="flex flex-col justify-center items-center gap-1 text-slate-300">D - {questinsList && questinsList.audience && questinsList.audience[3]} %</div>
                        </div>
                    </div>
                    <div className={`flex flex-col gap-1 justify-center items-center cursor-pointer ${questinsList?.audience ? 'hidden' : ''} ${lifeLineOpetion.fiftyFifty === false && 'greyscal'}`} onClick={callFifty}>
                        <div className={`h-[60px] w-[60px] border-[1px]  rounded-full flex justify-center items-center border-color `} style={{ color: '#7c9fec' }}>50:50</div>
                        <div style={{ color: 'white' }}>50:50</div>
                    </div>
                    <div className={`flex flex-col gap-1 justify-center items-center cursor-pointer  ${questinsList?.audience ? 'hidden' : ''} ${lifeLineOpetion.audience === false && 'greyscal'}`} onClick={callAudience}>
                        <div className="h-[60px] w-[60px] border-[1px]  rounded-full flex justify-center items-center " style={{ borderColor: 'white' }}>
                            <Audience />
                        </div>
                        <div style={{ color: 'white' }}>Audience poll</div>
                    </div>
                    <div className={`flex flex-col gap-1 justify-center items-center cursor-pointer  ${questinsList?.audience ? 'hidden' : ''} ${lifeLineOpetion.timefreez === false && 'greyscal'}`} onClick={callTimefreez}>
                        <div className="h-[60px] w-[60px] border-[1px]  rounded-full flex justify-center items-center " style={{ borderColor: 'white' }}>
                            <Timer />
                        </div>
                        <div style={{ color: 'white' }}>Freeze Timer</div>
                    </div>
                    <div className={`flex flex-col gap-1 justify-center items-center cursor-pointer  ${questinsList?.audience ? 'hidden' : ''} ${lifeLineOpetion.switch === false && 'greyscal'}`} onClick={callSwitch}>
                        <div className="h-[60px] w-[60px] border-[1px]  rounded-full flex justify-center items-center " style={{ borderColor: 'white' }}>
